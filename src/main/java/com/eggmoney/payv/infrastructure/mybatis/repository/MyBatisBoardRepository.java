 ...
 * - 책임: BoardRepository 인터페이스 구현 (MyBatis 기반).
 * - BoardRecord ↔ Board 변환을 담당.
 * 
 * 동작 방식:
 *   - findById(): Mapper 호출 후 Domain 변환
 *   - save(): 존재 여부 확인 후 insert/update 결정
 *   - findAll(): 전체 조회 후 Domain 변환
 *   - findByUser(): 특정 사용자 조회 후 Domain 변환
 * 
 * @author 한지원
 *
 */
...
public class MyBatisBoardRepository implements BoardRepository {
    ...
    public List<Board> findByUser(UserId userId) {
        return mapper.selectListByUser(userId.value()).stream()
                .map(this::toDomain)
                .collect(Collectors.toList());
    }
...
//          .type(BoardType.valueOf(record.getType())) // String → Enum
    ...
    private BoardRecord toRecord(Board board) {
        return BoardRecord.builder()
            .boardId(board.getId().value())
            .userId(board.getUserId().value())
